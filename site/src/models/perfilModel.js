...
function cadastrar(nome, email, senha, cpf, perfilAdministrador) {
    ...
    var instrucao = `
        INSERT INTO perfil (nome, email, senha, cpf, perfilAdministrador) VALUES ('${nome}', '${email}ss', '${senha}', '${cpf}' , ${perfilAdministrador});
    `;
    console.log("Executando a instrução SQL: \n" + instrucao);
    return database.executar(instrucao);
}
...
function alterarSenha(idUsuario, senhaNova, senhaAntiga) {
    ...
    const instrucao = `
        UPDATE perfil SET senha = '${senhaNova}' WHERE idPerfil = ${idUsuario} and senha = '${senhaAntiga}';
    `;
    console.log("Executando a instrução SQL: \n" + instrucao);
    return database.executar(instrucao);
}
